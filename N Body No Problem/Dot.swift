...
class DotNode: SKSpriteNode {
    ...
    init(color: UIColor, size: CGSize, initialPosition: CGPoint, withCameraPos: CGPoint ) {
        ...
        self.run(SKAction.sequence([
            ...
            SKAction.group([ SKAction.fadeAlpha(to: 1.0, duration: randSpeedTIme),            SKAction.move(to: overshootPosition, duration: randSpeedTIme) ]), SKAction.move(to: initialPosition, duration: 0.1)
, SKAction.run {
